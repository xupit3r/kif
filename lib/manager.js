...
function spawn (url) {
  ...
  debug(`spawning a thread to process ${url}`);
  ...
  return new Promise((resolve, reject) => {
    worker.once('message', ({ type, content }) => {
      if (type === messages.MESSAGE_SUMMARY) {
        ...
        events.emit(messages.MESSAGE_NEW_LINKS, content.links.map(link => {
          return {
            from: url,
            to: link
          };
...
module.exports = function manager () {
  while (spawnable()) {
    let link = LINKS_TO_PROCESS.shift();
    let threadId = uuid();
    let thread = spawn(link.to);
